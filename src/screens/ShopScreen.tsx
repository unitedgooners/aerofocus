...
export default function ShopScreen({ onBack, onUpgrade }: Props) {
  ...
  const { theme }                 = useThemeStore() // regular cabin theme — Shop feels like the rest of the app, not the Hangar
  ...
  const [tab, setTab]                   = useState<'aircraft' | 'themes'>('aircraft')
  ...
  // Owned aircraft are excluded entirely — once bought, it belongs in the Hangar, not here.
  ...
  const fleetFull = !isPremium && fleet.length >= FREE_TIER_AIRCRAFT_LIMIT
    ...
    <div style={{ minHeight: '100vh', background: theme.bg, transition: 'background 0.6s ease' }}>
      <div style={{ maxWidth: 480, margin: '0 auto', padding: `${space.xl}px ${space.lg}px 100px` }}>
        ...
        {!isPremium && (
          <button
            ...
            style={{
              ...
              border: `0.5px solid ${theme.gradientFrom}40`,
          ...
          >
            ...
            <span style={{ fontSize: font.xs, color: theme.text, lineHeight: 1.4, fontWeight: 600 }}>
              Want more aircraft? <span style={{ color: theme.textAccent }}>Upgrade to Premium</span> to unlock the full store.
            </span>
          </button>
        )}
        ...
        <div style={{
          ...
          {(['aircraft', 'themes'] as const).map(t => {
            const active = tab === t
            const count  = t === 'aircraft' ? shopItems.length : themeShopItems.length
            const label  = t === 'aircraft' ? 'Aircraft' : 'Themes'
            return (
...
function AircraftShopGrid({
  ...
  if (items.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: space.xxl, color: theme.textTertiary, fontSize: font.sm }}>
        You own every aircraft currently available. New ones coming soon ✈
      </div>
    )
  }
              ...
              {!isFounder && fleetFull && (
                <button
                  ...
                  style={{
                    ...
                    background: theme.bgWarning, color: theme.textWarning,
...
// ── Theme shop grid — boarding pass styles ──────────────────────────────────────
function ThemeShopGrid({
  ...
  const purchasable = items.filter(t => !ownedIds.has(t.id))

  if (purchasable.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: space.xxl, color: theme.textTertiary, fontSize: font.sm }}>
        You own every theme currently available ✈
      </div>
    )
  }

  const all = [...purchasable].sort((a, b) => {
    if (a.price === null && b.price !== null) return 1
    if (a.price !== null && b.price === null) return -1
    return (a.price ?? 0) - (b.price ?? 0)
  })
